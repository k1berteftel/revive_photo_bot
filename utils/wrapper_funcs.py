...
async def process_generate(msg: Message, sleep):
...
async def generate_wrapper(func, bot: Bot, user_id: int, sleep: int | float, *args) -> any:
    await bot.send_chat_action(user_id, "typing")
    msg = await bot.send_message(
        chat_id=user_id,
        text=_progress_text(1)
    )
    task = asyncio.create_task(process_generate(msg, sleep=sleep))
    try:
